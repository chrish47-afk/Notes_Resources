...
write.xlsx(location_data, "/ihme/scratch/users/chrish47/location_data.xlsx")
...
plot_p_final_1 <- read.xlsx("/ihme/scratch/users/chrish47/plot_p_final_1.xlsx")
plot_p_final_2 <- read.xlsx("/ihme/scratch/users/chrish47/plot_p_final_2.xlsx")

#### --- Plotting --- ############################################################################################################
# (1) Original plot, with everything plotted in one single stroke. Main plot(layers) and legends. 
# (2) WORK ON THIS PLOT. Plotting Main plot(layers) and legends independently. 
##################################################################################################################################
# Arranging the order of the sample size. From Smallest to Biggest Value. 
plot_p_final_1 <- plot_p_final_1 %>% arrange(desc(sample_size))

# (1)
### Original Plot -- Main Plots(and layeres) and legends are plotted together --- ###
ggplot() +
...
geom_point(data = plot_p_final_1,
           ...
           alpha = 0.1) +
  scale_size_continuous(range = c(10, 35)) + #changing the size of the sample size bubbles #Default range= for points is form 1 to 6(c(5, 12))
  labs(size = "Sample Size", color = "Numer of Cases") + #These are being addressed in the scale_##_manual() above.
...
geom_point(data = plot_p_final_1, 
  ...
  theme(legend.position = 'bottom', text = element_text(color = "black"), legend.key.size = unit(0.5, "cm"))
  
  
# Issues to fix - LEGENDS
# I need to find a proper way to align them independelty and adequately. 
...
#   # theme(legend.position = 'bottom', #or 'bottom'
#   #   #legend.direction = 'vertical',
#   #   legend.box = "horizontal",
#   #   legend.key.size = unit(0.5, "cm"), #changes legend size
#   #   legend.spacing.x = unit(0.3, 'cm'), #legend spacing
#   #   text = element_text(color = "black"),
#   #   legend.background = element_rect(fill="white", #This changes the legend's fill-in color!
...
# https://www.google.com/search?q=Warning+message%3A+Ignoring+unknown+aesthetics%3A+x%2C+y&rlz=1C1GCEA_enUS940US940&sourceid=chrome&ie=UTF-8
...
# https://stackoverflow.com/questions/64646003/how-to-create-choropleth-world-map-with-insets-where-necessary-in-r
#-- Patterns
#(1) http://sape.inf.usi.ch/quick-reference/ggplot2/shape
#(2) https://www.datanovia.com/en/blog/ggplot-point-shapes-best-tips/