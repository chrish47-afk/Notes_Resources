...
for index, row in dep_map.iterrows():
    ...
    for year in yearvals:
        ...
        change as needed """
        if (class_name not in ["abcdefghijk..."]): #Christian: This looks odd!
            adjust_job_list.append(job_name)
            call = ('sbatch -d {hj} -c 4 --mem=5G'
                    ' -p long.q -C archive'
                    ' -A proj_rgud'
                    ' -t 12:30:00'
                    ' -o {o}'
    ...
    for output_me in epi_upload_list:
        ...
        if (int(bundle_tuple[1]) > -1):
            ...
            call = ('sbatch -d {hj} -c 3 --mem=5G'
                    ...
                    ' {arg1} {arg2} "{arg3}" '.format(hj=hold, o=output_path, # Deleted decomp_step, no longer needed for elmo functions
