...
def upload_data(df, bundle, destination_file):
    if not df.empty:
        ...
        validate = elmo.validate_input_sheet(bundle, destination_file, 
            error_path)
        assert (validate['status'].item()=='passed')
        report = elmo.upload_bundle_data(bundle, destination_file)
...
def upload_maternal_epi_bundle(in_dir, acause, bundle):
    to_upload_dir = ('/home/j/WORK/12_bundle/{}/{}/'
                     '03_review/02_upload'.format(acause,bundle))
    ...
    before modification. """
    epi = elmo.get_bundle_data(bundle, export=True)
