...
class Ectopic(Base):
    ...
    complicated_subacute_prev_seq_me, bleedingonly_prev_uncomplicated_seq_me, bleedingonly_prev_complicated_seq_me, release_id):
        ...
        self.control_seq_me = uncomplicated_inc_seq_me #28983
        self.mild_seq_me = complicated_inc_seq_me # 28984
        self.mod_seq_me = complicated_acute_prev_seq_me #28985
        self.other_seq_me = uncomplicated_prev_pain_bleeding_seq_me #32537
        self.sev_seq_me = complicated_subacute_prev_seq_me #32539
        self.unc_bleedingonly_seq_me = bleedingonly_prev_uncomplicated_seq_me #32538
        self.com_bleedingonly_seq_me = bleedingonly_prev_complicated_seq_me #32540
    ...
    def run(self):
        ...
        # create new prevalence (from original ectopic pregnancy pipeline)
        duration = self.create_log_normal_draws(0.0082, 0.0055, 0.0110)
        #** Use log-normal for Ectopic, duration draws
        #** Use beta for Ectopic, severity draws
        new_prev = self.mul_draws(new_inc, duration)
        self.output(new_prev, output_me, 5)

        # generate severity draws - Ectopic: uncomplicated and complicated     
        uncomplicated = self.create_draws(0.87, 0.78, 0.92)
        complicated = self.create_draws(0.13, 0.08, 0.22)
        #** Use log-normal for Ectopic, duration draws
        #** Use beta for Ectopic, severity draws

        # squeeze severities - ectopic, two severity squeeze.
        uncomplicated, complicated = self.squeeze_severity_splits(uncomplicated, complicated)
        
        # generate duration draws for the two ectopic severity Duration groups.
        ...
        uncomplicated_subacute_dur_bleedingonly = self.create_log_normal_draws(17.32 / 365, 16.62 / 365, 18.02 / 365)
        print(uncomplicated_subacute_dur_bleedingonly[:5])
        print(uncomplicated_subacute_dur[:5])
        #Substracting Draws #
...
if __name__ == "__main__":
    ...
    if class_name == "Ectopic":
        ...
        model = Ectopic(cluster_dir, year, input_me, output_me, uncomplicated_inc_seq_me, complicated_inc_seq_me, complicated_acute_prev_seq_me,uncomplicated_prev_pain_bleeding_seq_me, complicated_subacute_prev_seq_me, bleedingonly_prev_uncomplicated_seq_me, bleedingonly_prev_complicated_seq_me, release_id)
    # uncomplicated_seq_me, complicated_inc_seq_me, complicated_acute_prev_seq_me, complicated_subacute_prev_seq_me
    else:
        raise ValueError('''Must be Ectopic''')
