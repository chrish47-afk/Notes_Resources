...
dt_combine_nids <- rbind(dt_19694_edit, dt_20036_edit, dt_20552_edit, dt_20792_edit, dt_20033_edit, dt_20900_edit, dt_29777_edit, dt_20465_edit, dt_20810_edit) %>% select(nid, location_id,location_name) %>% unique() #One file was created for Unique NIDs only and another was outputed to include and match Unique NIDs + Location(admin0 only).

location_collapsed <- location_data %>% select(location_id, location_name, ihme_loc_id, region_id, region_name)
location_collapsed <- location_collapsed %>% mutate(country = substr(ihme_loc_id, 1, 3))

dt_combine_nids_edit <- left_join(dt_combine_nids, location_collapsed, by = 'location_id') %>% select(nid, location_name.x, country) %>% dplyr::rename("map_id" = 'country')
dt_combine_nids_edit <- dt_combine_nids_edit %>% left_join(location_data %>% select(map_id, location_name), by = 'map_id')
dt_combine_nids_edit <- dt_combine_nids_edit %>% select(nid, map_id, location_name) %>% unique() %>% dplyr::rename('NID' = 'nid')

write.xlsx(dt_combine_nids_edit,'/ihme/scratch/users/chrish47/unique_nids_locations.xlsx')

datalandscaping <- read.xlsx("/ihme/scratch/users/chrish47/source_counts_mb/Clinical_data_review/Drafts_September_2022/DRAFT_Clinical_Data_09_2022.xlsx", startRow = 4)

combine_dt <- left_join(dt_combine_nids_edit, datalandscaping, by = "NID") %>% setorder(NID)
write.xlsx(combine_dt, "/ihme/scratch/users/chrish47/source_counts_mb/Clinical_data_review/Drafts_September_2022/DRAFT_Countries_Clinical_Data_09_2022.xlsx")
