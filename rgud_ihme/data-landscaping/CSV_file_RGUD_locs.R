...
for (i in seq_along(output_prevalence_mes)) {
  ...
  if (me == 1536) {
    ...
    dt_saved_draws_1536 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_1536.rds"))
    df_subset_1536 <- get_cols(dt_saved_draws_1536)
    df_subset_1536 <- df_subset_1536 %>% merge(location_data_draws, by = "location_id")
    
    # Melting data. Location_name will included super_region_name, region_name, and country_name.
    df_subset_melt_1536 <- df_subset_1536 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    df_subset_melt_1536 <- as.data.table(df_subset_melt_1536) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    ...
    dt_saved_draws_1537 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_1537.rds"))
    df_subset_1537 <- get_cols(dt_saved_draws_1537)
    df_subset_1537 <- df_subset_1537 %>% merge(location_data_draws, by = "location_id")
    
    # Melting data. Location_name will included super_region_name, region_name, and country_name.
    df_subset_melt_1537 <- df_subset_1537 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    df_subset_melt_1537 <- as.data.table(df_subset_melt_1537) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    
    # Adding Prevalence Rates, 1536 + 1537
    df_sum_1535_1537 <- df_subset_melt_1536 %>% select(location_id, location_name, year_id, age_group_id, starts_with("draw_")) %>% mutate(across(starts_with("draw_"), ~ . + df_subset_melt_1537[[cur_column()]]))
    
    # Summing prevalence rates for country, region, super region, and global
    df_sum_1535_1537_rates <- df_sum_1535_1537 %>%
      group_by(location_name, year_id, age_group_id) %>%
      summarise(
        across(starts_with("draw_"), ~ sum(.x, na.rm = TRUE)),
        .groups = "drop"
    ...
    dt_prevalence_rates <- mean_ui_columns(df_sum_1535_1537_rates, group_cols = c("year_id", "location_name", "age_group_id"))
    ...
    dt_prevalence_counts <- mean_ui_columns(dt_prevalence_counts, group_cols = c("year_id", "location_name", "age_group_id"))
    dt_prevalence_counts <- dt_prevalence_counts %>%
      merge(ages, by = "age_group_id") %>%
      arrange(year_id, age_group_id) %>%
    ...
    #### Prevalence Rate
    ### 27702 ----
    dt_saved_draws_27702 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_27702.rds"))
    df_subset_27702 <- get_cols(dt_saved_draws_27702)
    df_subset_27702 <- df_subset_27702 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_27702 <- df_subset_27702 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_27702 <- as.data.table(df_subset_melt_27702) %>% arrange(location_id, year_id, age_group_id)
    
    ### 27703 ----
    dt_saved_draws_27703 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_27703.rds"))
    df_subset_27703 <- get_cols(dt_saved_draws_27703)
    df_subset_27703 <- df_subset_27703 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_27703 <- df_subset_27703 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_27703 <- as.data.table(df_subset_melt_27703) %>% arrange(location_id, year_id, age_group_id)
    
    ### 27704 ----
    dt_saved_draws_27704 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_27704.rds"))
    df_subset_27704 <- get_cols(dt_saved_draws_27704)
    df_subset_27704 <- df_subset_27704 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_27704 <- df_subset_27704 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_27704 <- as.data.table(df_subset_melt_27704) %>% arrange(location_id, year_id, age_group_id)
    
    ### 27705 ----
    dt_saved_draws_27705 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_27705.rds"))
    df_subset_27705 <- get_cols(dt_saved_draws_27705)
    df_subset_27705 <- df_subset_27705 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_27705 <- df_subset_27705 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_27705 <- as.data.table(df_subset_melt_27705) %>% arrange(location_id, year_id, age_group_id)
    
    # Summing the draw columns across me_ids, 27702 + 27703 + 27704 + 27706
    df_sum_all <- df_subset_melt_27702 %>%
      select(location_id, location_name, year_id, age_group_id, starts_with("draw_")) %>%
      mutate(across(starts_with("draw_"), ~ 
                      . + df_subset_melt_27703[[cur_column()]]
                    + df_subset_melt_27704[[cur_column()]]
                    + df_subset_melt_27705[[cur_column()]]
      ))
    ...
    dt_prevalence_rates <- mean_ui_columns(df_sum_all_rates, group_cols = c("year_id", "location_name", "age_group_id")) %>%
      merge(ages, by = "age_group_id") %>%
      arrange(year_id, age_group_id) %>%
      mutate(
        Prevalence_rate = val,
    ...
    #### Prevalence Rate
    ### 28983 ----
    dt_saved_draws_28983 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_28983.rds"))
    df_subset_28983 <- get_cols(dt_saved_draws_28983)
    df_subset_28983 <- df_subset_28983 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_28983 <- df_subset_28983 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_28983 <- as.data.table(df_subset_melt_28983) %>% arrange(location_id, year_id, age_group_id)
    
    ### 28985 ----
    dt_saved_draws_28985 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_28985.rds"))
    df_subset_28985 <- get_cols(dt_saved_draws_28985)
    df_subset_28985 <- df_subset_28985 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_28985 <- df_subset_28985 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_28985 <- as.data.table(df_subset_melt_28985) %>% arrange(location_id, year_id, age_group_id)
    
    ### 28986 ----
    dt_saved_draws_28986 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_28986.rds"))
    #dt_saved_draws_28986 <- readRDS(paste0("/mnt/share/mmbrf/2A_improve_duration_severity/Ectopic_pregnancy_Natural_History_SR/Analysis/EP draws/EP_gbd_comp_sub_prev_draws.rds"))
    df_subset_28986 <- get_cols(dt_saved_draws_28986)
    df_subset_28986 <- df_subset_28986 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_28986 <- df_subset_28986 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_28986 <- as.data.table(df_subset_melt_28986) %>% arrange(location_id, year_id, age_group_id)
    
    # Summing Prevalence Rates, 28983 + 28985 + 28986
    df_sum_all <- df_subset_melt_28983 %>%
      select(location_id, location_name, year_id, age_group_id, starts_with("draw_")) %>%
      mutate(across(starts_with("draw_"), ~ 
                      . + df_subset_melt_28985[[cur_column()]] + df_subset_melt_28986[[cur_column()]]
      ))
    ...
    #### Prevalence Rate
    ### 32529 ----
    dt_saved_draws_32529 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_32529.rds"))
    df_subset_32529 <- get_cols(dt_saved_draws_32529)
    df_subset_32529 <- df_subset_32529 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_32529 <- df_subset_32529 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_32529 <- as.data.table(df_subset_melt_32529) %>% arrange(location_id, year_id, age_group_id)
    
    ### 32531 ----
    dt_saved_draws_32531 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_32531.rds"))
    df_subset_32531 <- get_cols(dt_saved_draws_32531)
    df_subset_32531 <- df_subset_32531 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_32531 <- df_subset_32531 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_32531 <- as.data.table(df_subset_melt_32531) %>% arrange(location_id, year_id, age_group_id)
    
    ### 32532 ----
    dt_saved_draws_32532 <- readRDS(paste0("/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_32532.rds"))
    df_subset_32532 <- get_cols(dt_saved_draws_32532)
    df_subset_32532 <- df_subset_32532 %>% merge(location_data_draws, by = "location_id")
    
    df_subset_melt_32532 <- df_subset_32532 %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name), values_to = "location_name") %>%
      select(-name)
    df_subset_melt_32532 <- as.data.table(df_subset_melt_32532) %>% arrange(location_id, year_id, age_group_id)
    
    # Summing the draw columns for me_ids, 32529 + 32531 + 32532
    df_sum_all <- df_subset_melt_32529 %>%
      select(location_id, location_name, year_id, age_group_id, starts_with("draw_")) %>%
      mutate(across(starts_with("draw_"), ~ 
                      . + df_subset_melt_32531[[cur_column()]] + df_subset_melt_32532[[cur_column()]]
      ))
...
WRA <- get_population(age_group_id=c(7:15), location_id="all", year_id="all", sex_id=2, with_ui=TRUE, release_id=16)
...
for (group_name in names(sequelae_groups)) {
  ...
  if(group_name == "group_10"){
    ...
    uncomplicated_prev<- readRDS(file = "/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_28983.rds")
    complicated_acute_prev<- readRDS(file = "/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_28985.rds")
    complicated_sub_prev<- readRDS(file = "/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_28986.rds")
    ...
    # Pulling population data for women of reproductive age
    #WRA <- get_population(age_group_id=c(7:15), location_id="all", year_id="all", sex_id=2, with_ui=TRUE, release_id=9)
    #WRA <- get_population(age_group_id=c(7:15), location_id="all", year_id="all", sex_id=2, with_ui=TRUE, release_id=16)
    ...
    # Rates
    work_uncomplicated_prev_rates <- work_uncomplicated_prev_rates %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    work_uncomplicated_prev_rates <- as.data.table(work_uncomplicated_prev_rates) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    
    work_complicated_acute_prev_rates <- work_complicated_acute_prev_rates %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    work_complicated_acute_prev_rates <- as.data.table(work_complicated_acute_prev_rates) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    
    work_complicated_sub_prev_rates <- work_complicated_sub_prev_rates %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    work_complicated_sub_prev_rates <- as.data.table(work_complicated_sub_prev_rates) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    
    
    # Counts
    work_uncomplicated_prev_counts <- work_uncomplicated_prev_counts %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    work_uncomplicated_prev_counts <- as.data.table(work_uncomplicated_prev_counts) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing

    work_complicated_acute_prev_counts <- work_complicated_acute_prev_counts %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    work_complicated_acute_prev_counts <- as.data.table(work_complicated_acute_prev_counts) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    
    work_complicated_sub_prev_counts <- work_complicated_sub_prev_counts %>%
      tidyr::pivot_longer(cols = c(global, super_region_name, region_name, country_name),  # Columns to stack
                          values_to = "location_name"                              # New column with the value
      ) %>% select(-name)
    work_complicated_sub_prev_counts <- as.data.table(work_complicated_sub_prev_counts) %>% arrange(location_id) # Including arrange to make sure they are aligned the same for summing
    ...
    # Summing prevalence rates and counts for country, region, super region, and global
    # Rates
    df_sum_all_rates <- df_sum_all_rates %>%
      group_by(location_name, year_id, age_group_id) %>%
      summarise(
        across(starts_with("draw_"), ~ sum(.x, na.rm = TRUE)),
        .groups = "drop"
    ...
    # ADDING EMILYs code for Usability and ease of use. ---------------
    # 32529, 32531,32532
    uncomplicated_prev<- readRDS(file = "/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_32529.rds")
    complicated_acute_prev<- readRDS(file = "/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_32531.rds")
    complicated_sub_prev<- readRDS(file = "/mnt/team/rgud/pub/users/chrish47/RDS_nf_maternal_prevalence/me_32532.rds")
...
write.csv(final_combined_table, "/mnt/team/rgud/pub/users/chrish47/CSV_files/final_maternal_nonfatal_table_locs.csv", row.names = FALSE)
write.csv(final_combined_table, "/mnt/share/mmbrf/Deliverables/April_29_2025_maternal_nonfatal_table_locs.csv", row.names = FALSE)
...
write.csv(final_combined_table, "/mnt/share/mmbrf/Deliverables/April15_2025_maternal_nonfatal_table_gdm.csv", row.names = FALSE)
...
write.csv(final_incidence_pct_change, "/mnt/share/mmbrf/Deliverables/April_23_2025_gdm_pct_2023_1990_10_54.csv", row.names = FALSE)
